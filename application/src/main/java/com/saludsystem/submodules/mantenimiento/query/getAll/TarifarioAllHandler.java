...
public class TarifarioAllHandler
{
	...
	public ListResponse<TarifarioDTO> execute(UUID hospitalId, PaginationRequest paginationRequest)
	{
		...
		List<TarifarioDTO> data = result.getData().stream().map(tarifarioMapper::toDto).toList();
		return new ListResponse<>(data, result.getTotalElements(), result.getCurrentPage(), result.getCurrentPage());
	}
}