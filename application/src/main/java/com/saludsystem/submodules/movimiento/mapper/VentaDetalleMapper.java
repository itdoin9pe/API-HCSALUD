...
public class VentaDetalleMapper
{
	...
	{
		return new VentaDetalle(null, createCommand.getProductoId(), createCommand.getVentaId(),
				createCommand.getCodigoProducto(), createCommand.getCantidad(), createCommand.getPrecioUnitario(),
				createCommand.getSubtotal());
	}

	public VentaDetalle fromUpdateDto(UUID uuid, VentaDetalleEditCommand editCommand)
	{
		return new VentaDetalle(null, editCommand.getProductoId(), editCommand.getVentaId(),
				editCommand.getCodigoProducto(), editCommand.getCantidad(), editCommand.getPrecioUnitario(),
				editCommand.getSubtotal());
	}

	public VentaDetalleDTO toDto(VentaDetalle model)
	{
		return new VentaDetalleDTO(model.getVentaDetalleId(), model.getVentaId(), model.getProductoId(),
				model.getCodigoProducto(), model.getCantidad(), model.getPrecioUnitario().doubleValue(),
