...
public class VentaMapper
{
	...
	{
		List<VentaDetalle> detalles = createCommand.getDetalles().stream().map(detalleMapper::fromCreateDto)
	...
	public Venta fromUpdateDto(UUID uuid, VentaEditCommand editCommand)
	{
		List<VentaDetalle> detalles = editCommand.getDetalles().stream()
				.map(d -> detalleMapper.fromUpdateDto(d.getVentaDetalleId(), d)).collect(Collectors.toList());
	...
	{
		return new VentaDTO(model.getVentaId(), model.getSerie(), model.getSecuencia(), model.getFecha(),
				model.getTipoBeneficiario(), model.getBeneficiarioDocumento(), model.getBeneficiarioDireccion(),
				model.getTipoPagoId(), model.getObservacion(), model.getMetodoPago(), model.getMontoRecibido(),
				model.getVuelto(), model.getTotal(), model.getTipoDocumentoId(), model.getAlmacenId(),
				model.getBeneficiarioId(), model.getTipoTarjetaId(), model.getTipoMonedaId(), model.getEstado(),
