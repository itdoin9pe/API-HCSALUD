...
public class VentaDetalleAllHandler
{
	...
	public ListResponse<VentaDetalleDTO> execute(UUID hospitalId, PaginationRequest paginationRequest)
	{
		...
		List<VentaDetalleDTO> data = result.getData().stream().map(ventaDetalleMapper::toDto).toList();
		return new ListResponse<VentaDetalleDTO>(data, result.getTotalElements(), result.getTotalPages(),
