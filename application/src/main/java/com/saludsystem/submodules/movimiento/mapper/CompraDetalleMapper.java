...
public class CompraDetalleMapper
{
	...
	{
		return new CompraDetalle(null, createCommand.getCompraId(), createCommand.getProductoId(),
				createCommand.getCantidad(), createCommand.getPrecioUnitario(), createCommand.getIgv(),
				createCommand.getPrecioVenta(), createCommand.getSubtotal());
	}

	public CompraDetalle fromUpdateDto(UUID id, CompraDetalleEditCommand editCommand)
	{
		return new CompraDetalle(id, editCommand.getCompraId(), editCommand.getProductoId(), editCommand.getCantidad(),
				editCommand.getPrecioUnitario(), editCommand.getIgv(), editCommand.getPrecioVenta(),
				editCommand.getSubtotal());
	}

	public CompraDetalleDTO toDto(CompraDetalle model)
	{
		return new CompraDetalleDTO(model.getCompraDetalleId(), model.getCompraId(), model.getProductoId(),
				model.getCantidad(), model.getPrecioUnitario(), model.getIgv(), model.getPrecioVenta(),
				model.getSubtotal());
	}
}