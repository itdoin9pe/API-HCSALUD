...
public class AlmacenAllHandler
{
	...
	public ListResponse<AlmacenDTO> execute(UUID hospitalId, PaginationRequest paginationRequest)
	{
		...
		List<AlmacenDTO> data = result.getData().stream().map(almacenMapper::toDto).toList();
		return new ListResponse<AlmacenDTO>(data, result.getTotalElements(), result.getTotalPages(),
