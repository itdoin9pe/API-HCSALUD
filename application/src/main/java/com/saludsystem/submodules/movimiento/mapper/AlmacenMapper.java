...
public class AlmacenMapper
{
	...
	{
		return new Almacen(null, createCommand.getNombre(), createCommand.getEstado(), createCommand.getSedeId(),
				createCommand.getSucursalId());
	}

	public Almacen fromUpdateDto(UUID uuid, AlmacenEditCommand editCommand)
	{
		return new Almacen(uuid, editCommand.getNombre(), editCommand.getEstado(), editCommand.getSedeId(),
				editCommand.getSucursalId());
	}

	public AlmacenDTO toDto(Almacen model)
	{
		return new AlmacenDTO(model.getAlmacenId(), model.getNombre(), model.getEstado(), model.getSedeId(),
				model.getSucursalId());
	}
}