...
public class EstadoCuentaMapper
{
	...
	{
		return new EstadoCuenta(null, // ID generado por persistencia
				createCommand.getPacienteId(), createCommand.getFechaApertura(), createCommand.getEstado(),
				createCommand.getTotalHospitalizacion(), createCommand.getTotalMedicamentosEstudios(),
				createCommand.getTotalPagado(), createCommand.getSaldoPendiente(), Collections.emptyList(),
				Collections.emptyList(), Collections.emptyList());
	}

	public EstadoCuenta fromUpdateDto(UUID uuid, EstadoCuentaEditCommand editCommand)
	{
		return new EstadoCuenta(uuid, editCommand.getPacienteId(), editCommand.getFechaApertura(),
				editCommand.getEstado(), editCommand.getTotalHospitalizacion(),
				editCommand.getTotalMedicamentosEstudios(), editCommand.getTotalPagado(),
				editCommand.getSaldoPendiente(), Collections.emptyList(), Collections.emptyList(),
				Collections.emptyList());
	}

	public EstadoCuentaDTO toDto(EstadoCuenta model)
	{
		return new EstadoCuentaDTO(model.getEstadoCuentaId(), model.getPacienteId(), model.getFechaApertura(),
				model.getEstado(), model.getTotalHospitalizacion(), model.getTotalMedicamentosEstudios(),
				model.getTotalPagado(), model.getSaldoPendiente(),
				mapCostosHospitalizacion(model.getCostosHospitalizacion()),
				mapDetallesMedicamentosEstudios(model.getDetallesMedicamentosEstudios()), mapPagos(model.getEPagos()));
	}
	...
	private List<CostoHospitalizacionDTO> mapCostosHospitalizacion(List<CostoHospitalizacion> list)
	{
		...
		return list.stream()
				.map(c -> new CostoHospitalizacionDTO(c.getPec_costoHospitalizacionId(), c.getEstadoCuentaId(),
						c.getPacienteId(), c.getFechaIngreso(), c.getFechaAlta(), c.getTipoHabitacion(),
						c.getCostoPorDia(), c.getCantidadDias(), c.getTotalCosto()))
				.toList();
	}
	...
	private List<DetalleMedicamentoEstudioDTO> mapDetallesMedicamentosEstudios(List<DetalleMedicamentoEstudio> list)
	{
		...
		return list.stream()
				.map(d -> new DetalleMedicamentoEstudioDTO(d.getPec_detalleMedicamentoId(), d.getPacienteId(),
						d.getEstadoCuentaId(), d.getTipo(), d.getDescripcion(), d.getCantidad(), d.getCostoUnitario(),
						d.getTotalCosto()))
				.toList();
	}
	...
	private List<EPagoDTO> mapPagos(List<EPago> list)
	{
		...
		return list.stream().map(p -> new EPagoDTO(p.getPecPagoId(), p.getPacEstadoCuentaId(), p.getFechaPago(),
				p.getMonto(), p.getMetodoPago(), p.getReferenciaPago())).toList();
	}
}