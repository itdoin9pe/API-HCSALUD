...
public class RecetaMapper
{
	...
	{
		return new Receta(null, createCommand.getFecha(), createCommand.getDoctorId(), createCommand.getPacienteId(),
				createCommand.getObservaciones(), createCommand.getEstado(),
	...
	{
		return new Receta(id, editCommand.getFecha(), editCommand.getDoctorId(), editCommand.getPacienteId(),
				editCommand.getObservaciones(), editCommand.getEstado(),
				medicamentoMapper.fromDtoList(editCommand.getMedicamentos()));
	}

	public RecetaDTO toDto(Receta model)
	{
		return new RecetaDTO(model.getPacienteRecetaId(), model.getDoctorId(), model.getPacienteId(), model.getFecha(),
				model.getObservaciones(), model.getEstado(),
