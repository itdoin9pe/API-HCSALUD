...
public class EstudioMedicoMapper
{
	...
	{
		return new EstudioMedico(null, editCommand.getTipo(), editCommand.getDescripcion(), editCommand.getEstado(),
				editCommand.getPacienteId(), editCommand.getDoctorId(), editCommand.getPacienteEstudioResultadoId());
	}

	public EstudioMedicoDTO toDto(EstudioMedico model)
	{
		return new EstudioMedicoDTO(model.getId(), model.getTipo(), model.getDescripcion(), model.getEstado(),
				model.getPacienteId(), model.getDoctorId(), model.getEstudioResultadoId());
	}
}