...
public class EPagoMapper {
    ...
    public EPago fromUpdateDto(UUID uuid, EPagoEditCommand editCommand) {
        ...
        return new EPago(uuid, editCommand.getPacEstadoCuentaId(), editCommand.getFechaPago(),
                editCommand.getMonto(), editCommand.getMetodoPago(), editCommand.getReferenciaPago());

    }

    public EPagoDTO toDto(EPago model) {

        return new EPagoDTO(model.getPecPagoId(),
                model.getPacEstadoCuentaId(),
                model.getFechaPago(),
                model.getMonto(),
                model.getMetodoPago(),
                model.getReferenciaPago());
