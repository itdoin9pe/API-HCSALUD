...
public class ExploracionFisicaMapper
{
	...
	{
		return new ExploracionFisica(null, createCommand.getPacienteId(), createCommand.getPresionArterial(),
				createCommand.getPulso(), createCommand.getTemperatura(), createCommand.getFrecuenciaCardiaca(),
				createCommand.getFrecuenciaRespiratoria(), createCommand.getPeso(), createCommand.getTalla(),
				createCommand.getMasa(), createCommand.getExamenClinico(), createCommand.getComplementoExamen());
	}
	...
	{
		return new ExploracionFisicaDTO(model.getPacienteExploracionId(), model.getPacienteId(),
				model.getPresionArterial(), model.getPulso(), model.getTemperatura(), model.getFrecuenciaCardiaca(),
				model.getFrecuenciaRespiratoria(), model.getPeso(), model.getTalla(), model.getMasa(),
				model.getExamenClinico(), model.getComplementoExamen());
	}
}