...
public class DetalleMedicamentoEstudioAllHandler {
    ...
    public DetalleMedicamentoEstudioAllHandler(DetalleMedicamentoEstudioDao detalleMedicamentoEstudioDao, DetalleMedicamentoEstudioMapper detalleMedicamentoEstudioMapper) {
    ...
    public ListResponse<DetalleMedicamentoEstudioDTO> execute(UUID hospitalId, PaginationRequest paginationRequest) {
        ...
        var result = detalleMedicamentoEstudioDao.getAll(hospitalId, paginationRequest.getPage(), paginationRequest.getRows());

        List<DetalleMedicamentoEstudioDTO> data = result.getData()
                .stream()
                .map(detalleMedicamentoEstudioMapper::toDto)
                .toList();
