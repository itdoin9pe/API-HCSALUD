...
public class PlanTratamientoMapper
{
	...
	{
		return new PlanTratamiento(null, createCommand.getPacienteId(), createCommand.getDoctorId(),
				createCommand.getEspecialidadId(), createCommand.getMedidaId(), createCommand.getFechaInicio(),
				createCommand.getFechaFin());
	}

	public PlanTratamiento fromUpdateDto(UUID uuid, PlanTratamientoEditCommand editCommand)
	{
		return new PlanTratamiento(uuid, editCommand.getPacienteId(), editCommand.getDoctorId(),
				editCommand.getEspecialidadId(), editCommand.getMedidaId(), editCommand.getFechaInicio(),
				editCommand.getFechaFin());
	}

	public PlanTratamientoDTO toDto(PlanTratamiento model)
	{
		return new PlanTratamientoDTO(model.getPacientePlanTratamientoId(), model.getPacienteId(), model.getDoctorId(),
				model.getEspecialidadId(), model.getMedidaId(), model.getFechaInicio(), model.getFechaFin());
	}
}