...
public class ConsultaMapper
{
	...
	public Consulta fromUpdateDto(UUID uuid, ConsultaEditCommand editCommand)
	{
		return new Consulta(uuid, editCommand.getPacienteId(), editCommand.getEspecialidadId(),
				editCommand.getMotivoConsulta(), editCommand.getDiagnosticoPresuntivo(), editCommand.isTieneAlergia(),
				editCommand.getTieneAlergiaTexto(), editCommand.isTomaMedicamento(),
				...
				editCommand.isTransfusionesSanguineas(), editCommand.isHistorialPsiquiatrico(), editCommand.isFuma(),
				editCommand.isConsumeAlcohol(), editCommand.getActividadFisica(), editCommand.isEmbarazada(),
				editCommand.getUltimaMenstruacion());
	}

	public ConsultaDTO toDto(Consulta model)
	{
		return new ConsultaDTO(model.getPacienteConsultaId(), model.getPacienteId(), model.getEspecialidadId(),
				model.getMotivoConsulta(), model.getDiagnosticoPresuntivo(), model.isTieneAlergia(),
				model.getTieneAlergiaTexto(), model.isTomaMedicamento(), model.getTomaMedicamentoTexto(),
				model.isCirugiasPrevias(), model.getCirugiasPreviasTexto(), model.getEnfermedadesCronicas(),
				model.isTransfusionesSanguineas(), model.isHistorialPsiquiatrico(), model.isFuma(),
				model.isConsumeAlcohol(), model.getActividadFisica(), model.isEmbarazada(),
				model.getUltimaMenstruacion());
	}
}