...
public class ProcedimientoMapper
{
	...
	{
		return new Procedimiento(null, createCommand.getPacienteId(), createCommand.getDoctorId(),
				createCommand.getTipoProcedimiento(), createCommand.getDescripcion(), createCommand.getFecha(),
				createCommand.getResultado());
	}

	public Procedimiento fromUpdateDto(UUID uuid, ProcedimientoEditCommand editCommand)
	{
		return new Procedimiento(uuid, editCommand.getPacienteId(), editCommand.getDoctorId(),
				editCommand.getTipoProcedimiento(), editCommand.getDescripcion(), editCommand.getFecha(),
				editCommand.getResultado());
	}

	public ProcedimientoDTO toDto(Procedimiento model)
	{
		return new ProcedimientoDTO(model.getProcedimientoId(), model.getPacienteId(), model.getDoctorId(),
				model.getTipoProcedimiento(), model.getDescripcion(), model.getFecha(), model.getResultado());
	}
}