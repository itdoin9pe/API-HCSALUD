...
public class RolMysqlDao implements RolDao
{
	...
	{
		return roleJpaRepository.findById(uuid).map(RolDboMapper::toDomain)
	...
	public ListResponse<Rol> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Rol> data = pageResult.getContent().stream().map(RolDboMapper::toDomain).toList();
	...
	{
		return roleJpaRepository.findAll().stream().map(RolDboMapper::toDomain).toList();
	}
}
