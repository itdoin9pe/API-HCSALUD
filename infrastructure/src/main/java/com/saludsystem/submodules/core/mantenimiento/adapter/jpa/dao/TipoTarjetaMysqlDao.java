...
public class TipoTarjetaMysqlDao implements TipoTarjetaDao
{
	...
	public ListResponse<TipoTarjeta> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<TipoTarjeta> data = pageResult.getContent().stream().map(TipoTarjetaDboMapper::toDomain).toList();
	...
	{
		return tipoTarjetaJpaRepository.findAll().stream().map(TipoTarjetaDboMapper::toDomain).toList();
	}
}
