...
public class DetalleMedicamentoEstudioMysqlDao implements DetalleMedicamentoEstudioDao
{
	...
	public DetalleMedicamentoEstudioMysqlDao(
	...
	{
		return detalleMedicamentoEstudioJpaRepository.findById(uuid).map(DetalleMedicamentoEstudioDboMapper::toDomain)
	...
	public ListResponse<DetalleMedicamentoEstudio> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<DetalleMedicamentoEstudio> data = pageResult.getContent().stream()
				.map(DetalleMedicamentoEstudioDboMapper::toDomain).toList();
	...
	{
		return detalleMedicamentoEstudioJpaRepository.findAll().stream()
				.map(DetalleMedicamentoEstudioDboMapper::toDomain).toList();
	}
}
