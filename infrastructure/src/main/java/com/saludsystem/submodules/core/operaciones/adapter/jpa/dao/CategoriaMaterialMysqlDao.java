...
public class CategoriaMaterialMysqlDao implements CategoriaMaterialDao
{
	...
	public ListResponse<CategoriaMaterial> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<CategoriaMaterial> data = pageResult.getContent().stream().map(CategoriaMaterialDboMapper::toDomain)
				.toList();
	...
	{
		return categoriaMatJpaRepository.findAll().stream().map(CategoriaMaterialDboMapper::toDomain).toList();
	}
}
