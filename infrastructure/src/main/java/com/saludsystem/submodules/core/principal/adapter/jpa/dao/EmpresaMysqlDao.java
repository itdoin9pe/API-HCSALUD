...
public class EmpresaMysqlDao implements EmpresaDao
{
	...
	public ListResponse<Empresa> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Empresa> data = pageResult.getContent().stream().map(EmpresaDboMapper::toDomain).toList();
	...
	{
		return empresaJpaRepository.findAll().stream().map(EmpresaDboMapper::toDomain).toList();
	}
}
