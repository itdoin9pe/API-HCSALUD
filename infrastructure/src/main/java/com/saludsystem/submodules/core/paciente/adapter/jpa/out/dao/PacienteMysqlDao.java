...
public class PacienteMysqlDao implements PacienteDao
{
	...
	{
		return pacienteJpaRepository.findById(uuid).map(PacienteDboMapper::toDomain)
	...
	public ListResponse<Paciente> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Paciente> data = pageResult.getContent().stream().map(PacienteDboMapper::toDomain).toList();
	...
	{
		return pacienteJpaRepository.findAll().stream().map(PacienteDboMapper::toDomain).toList();
	}
}
