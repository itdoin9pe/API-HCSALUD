...
public class TipoPagoMysqlDao implements TipoPagoDao
{
	...
	{
		return tipoPagoJpaRepository.findById(uuid).map(TipoPagoDboMapper::toDomain)
	...
	public ListResponse<TipoPago> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<TipoPago> data = pageResult.getContent().stream().map(TipoPagoDboMapper::toDomain).toList();
	...
	{
		return tipoPagoJpaRepository.findAll().stream().map(TipoPagoDboMapper::toDomain).toList();
	}
}
