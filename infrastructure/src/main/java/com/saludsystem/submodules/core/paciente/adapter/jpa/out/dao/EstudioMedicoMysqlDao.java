...
public class EstudioMedicoMysqlDao implements EstudioMedicoDao
{
	...
	{
		return estudioMedicoJpaRepository.findById(id).map(EstudioMedicoDboMapper::toDomain)
	...
	public ListResponse<EstudioMedico> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<EstudioMedico> data = pageResult.getContent().stream().map(EstudioMedicoDboMapper::toDomain).toList();
	...
	{
		return estudioMedicoJpaRepository.findAll().stream().map(EstudioMedicoDboMapper::toDomain).toList();
	}
}
