...
public class AseguradoraMysqlDao implements AseguradoraDao
{
	...
	public ListResponse<Aseguradora> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Aseguradora> data = pageResult.getContent().stream().map(AseguradoraDboMapper::toDomain).toList();
	...
	{
		return aseguradoraJpaRepository.findAll().stream().map(AseguradoraDboMapper::toDomain).toList();
	}
}
