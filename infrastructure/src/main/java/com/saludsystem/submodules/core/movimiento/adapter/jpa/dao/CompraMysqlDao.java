...
public class CompraMysqlDao implements CompraDao
{
	...
	{
		return compraJpaRepository.findById(uuid).map(CompraDboMapper::toDomain)
	...
	public ListResponse<Compra> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Compra> data = pageResult.getContent().stream().map(CompraDboMapper::toDomain).toList();
	...
	{
		return compraJpaRepository.findAll().stream().map(CompraDboMapper::toDomain).toList();
	}
}
