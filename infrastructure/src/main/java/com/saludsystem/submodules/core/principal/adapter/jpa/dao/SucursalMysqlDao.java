...
public class SucursalMysqlDao implements SucursalDao
{
	...
	{
		return sucursalJpaRepository.findById(uuid).map(SucursalDboMapper::toDomain)
	...
	public ListResponse<Sucursal> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Sucursal> data = pageResult.getContent().stream().map(SucursalDboMapper::toDomain).toList();
	...
	{
		return sucursalJpaRepository.findAll().stream().map(SucursalDboMapper::toDomain).toList();
	}
}
