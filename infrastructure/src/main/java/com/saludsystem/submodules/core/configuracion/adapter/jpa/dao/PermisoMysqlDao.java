...
public class PermisoMysqlDao implements PermisoDao
{
	...
	{
		return permisoJpaRepository.findById(uuid).map(PermisoDboMapper::toDomain)
	...
	public ListResponse<Permiso> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Permiso> data = pageResult.getContent().stream().map(PermisoDboMapper::toDomain).toList();
	...
	{
		return permisoJpaRepository.findAll().stream().map(PermisoDboMapper::toDomain).toList();
	}
}
