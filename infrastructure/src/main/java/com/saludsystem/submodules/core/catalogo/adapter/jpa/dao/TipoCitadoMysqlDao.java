...
public class TipoCitadoMysqlDao implements TipoCitadoDao
{
	...
	public ListResponse<TipoCitado> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<TipoCitado> data = pageResult.getContent().stream().map(TipoCitadoDboMapper::toDomain).toList();
	...
	{
		return tipoCitadoJpaRepository.findAll().stream().map(TipoCitadoDboMapper::toDomain).toList();
	}
}
