...
public class EvolucionMysqlDao implements EvolucionDao
{
	...
	{
		return evolucionJpaRepository.findById(uuid).map(EvolucionDboMapper::toDomain)
	...
	public ListResponse<Evolucion> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Evolucion> data = pageResult.getContent().stream().map(EvolucionDboMapper::toDomain).toList();
	...
	{
		return evolucionJpaRepository.findAll().stream().map(EvolucionDboMapper::toDomain).toList();
	}
}
