...
public class TipoMaterialMysqlDao implements TipoMaterialDao
{
	...
	{
		return tipoMaterialJpaRepository.findById(uuid).map(TipoMaterialDboMapper::toDomain)
	...
	public ListResponse<TipoMaterial> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<TipoMaterial> data = pageResult.getContent().stream().map(TipoMaterialDboMapper::toDomain).toList();
	...
	{
		return tipoMaterialJpaRepository.findAll().stream().map(TipoMaterialDboMapper::toDomain).toList();
	}
}
