...
public class VentaMysqlDao implements VentaDao
{
	...
	{
		return ventaJpaRepository.findById(uuid).map(VentaDboMapper::toDomain)
	...
	public ListResponse<Venta> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Venta> data = pageResult.getContent().stream().map(VentaDboMapper::toDomain).toList();
	...
	{
		return ventaJpaRepository.findAll().stream().map(VentaDboMapper::toDomain).toList();
	}
}
