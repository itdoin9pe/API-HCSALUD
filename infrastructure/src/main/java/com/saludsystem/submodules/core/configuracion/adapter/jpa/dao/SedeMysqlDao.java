...
public class SedeMysqlDao implements SedeDao
{
	...
	{
		return sedeJpaRepository.findById(uuid).map(SedeDboMapper::toDomain)
	...
	public ListResponse<Sede> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Sede> data = pageResult.getContent().stream().map(SedeDboMapper::toDomain).toList();
	...
	{
		return sedeJpaRepository.findAll().stream().map(SedeDboMapper::toDomain).toList();
	}
}
