...
public class EstadoCuentaMysqlDao implements EstadoCuentaDao
{
	...
	{
		return estadoCuentaJpaRepository.findById(uuid).map(EstadoCuentaDboMapper::toDomain)
	...
	public ListResponse<EstadoCuenta> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<EstadoCuenta> data = pageResult.getContent().stream().map(EstadoCuentaDboMapper::toDomain).toList();
	...
	{
		return estadoCuentaJpaRepository.findAll().stream().map(EstadoCuentaDboMapper::toDomain).toList();
	}
}
