...
public class EstudioResultadoMysqlDao implements EstudioResultadoDao
{
	...
	{
		return estudioResultadoJpaRepository.findById(id).map(EstudioResultadoDboMapper::toDomain)
	...
	public ListResponse<EstudioResultado> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<EstudioResultado> data = pageResult.getContent().stream().map(EstudioResultadoDboMapper::toDomain)
				.toList();
	...
	{
		return estudioResultadoJpaRepository.findAll().stream().map(EstudioResultadoDboMapper::toDomain).toList();
	}
}
