...
public class MonedaMysqlDao implements MonedaDao
{
	...
	public ListResponse<Moneda> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Moneda> data = pageResult.getContent().stream().map(MonedaDboMapper::toDomain).toList();
	...
	{
		return monedaJpaRepository.findAll().stream().map(MonedaDboMapper::toDomain).toList();
	}
}
