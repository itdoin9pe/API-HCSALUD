...
public class InformacionClinicaQueryController
{
	...
	public InformacionClinicaQueryController(
		InformacionClinicaByIdHandler byIdHandler,
		InformacionClinicaListHandler listHandler,
		InformacionClinicaAllHandler allHandler)
	{
	...
	@GetMapping("/GetById/{informacionClinicaId}")
	public InformacionClinicaDTO getById(@PathVariable UUID informacionClinicaId)
	{
		return byIdHandler.execute(informacionClinicaId);
	}

	@GetMapping("/GetAll")
	@ApiResponses(value =
	{ @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Operación exitosa", content = @Content(mediaType = "application/json", schema = @Schema(implementation = InformacionClinicaListResponse.class))) })
