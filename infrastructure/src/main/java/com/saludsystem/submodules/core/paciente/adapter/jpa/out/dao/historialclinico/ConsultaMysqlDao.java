...
public class ConsultaMysqlDao implements ConsultaDao
{
	...
	{
		return consultaJpaRepository.findById(uuid).map(ConsultaDboMapper::toDomain)
	...
	public ListResponse<Consulta> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Consulta> data = pageResult.getContent().stream().map(ConsultaDboMapper::toDomain).toList();
	...
	{
		return consultaJpaRepository.findAll().stream().map(ConsultaDboMapper::toDomain).toList();
	}
}
