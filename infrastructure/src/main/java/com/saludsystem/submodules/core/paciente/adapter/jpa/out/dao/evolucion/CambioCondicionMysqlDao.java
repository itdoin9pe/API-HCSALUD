...
public class CambioCondicionMysqlDao implements CambioCondicionDao
{
	...
	{
		return cambioCondicionJpaRepository.findById(id).map(CambioCondicionDboMapper::toDomain)
	...
	public ListResponse<CambioCondicion> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<CambioCondicion> data = pageResult.getContent().stream().map(CambioCondicionDboMapper::toDomain).toList();
	...
	{
		return cambioCondicionJpaRepository.findAll().stream().map(CambioCondicionDboMapper::toDomain).toList();
	}
}
