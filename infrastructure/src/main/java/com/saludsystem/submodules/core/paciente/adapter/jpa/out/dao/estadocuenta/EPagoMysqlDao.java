...
public class EPagoMysqlDao implements EPagoDao
{
	...
	{
		return ePagoJpaRepository.findById(uuid).map(EPagoDboMapper::toDomain)
	...
	public ListResponse<EPago> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<EPago> data = pageResult.getContent().stream().map(EPagoDboMapper::toDomain).toList();
	...
	{
		return ePagoJpaRepository.findAll().stream().map(EPagoDboMapper::toDomain).toList();
	}
}
