...
public class InterconsultaMysqlDao implements InterconsultaDao
{
	...
	{
		return interconsultaJpaRepository.findAll().stream().map(InterconsultaDboMapper::toDomain).toList();
	}

	@Override
	public Interconsulta getById(UUID uuid)
	{
		return interconsultaJpaRepository.findById(uuid).map(InterconsultaDboMapper::toDomain)
	...
	public ListResponse<Interconsulta> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Interconsulta> data = pageResult.getContent().stream().map(InterconsultaDboMapper::toDomain).toList();
