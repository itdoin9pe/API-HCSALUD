...
public class MarcaMysqlDao implements MarcaDao
{
	...
	{
		return marcaJpaRepository.findById(uuid).map(MarcaDboMapper::toDomain)
	...
	public ListResponse<Marca> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Marca> data = pageResult.getContent().stream().map(MarcaDboMapper::toDomain).toList();
	...
	{
		return marcaJpaRepository.findAll().stream().map(MarcaDboMapper::toDomain).toList();
	}
}
