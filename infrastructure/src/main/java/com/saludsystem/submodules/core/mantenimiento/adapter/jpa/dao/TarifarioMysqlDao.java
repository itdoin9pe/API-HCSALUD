...
public class TarifarioMysqlDao implements TarifarioDao
{
	...
	public ListResponse<Tarifario> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Tarifario> data = pageResult.getContent().stream().map(TarifarioDboMapper::toDomain).toList();
	...
	{
		return tarifarioJpaRepository.findAll().stream().map(TarifarioDboMapper::toDomain).toList();
	}
}
