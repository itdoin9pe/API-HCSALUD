...
public class CostoTratamientoMysqlDao implements CostoTratamientoDao
{
	...
	{
		return costoTratamientoJpaRepository.findById(uuid).map(CostoTratamientoDboMapper::toDomain)
	...
	public ListResponse<CostoTratamiento> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<CostoTratamiento> data = pageResult.getContent().stream().map(CostoTratamientoDboMapper::toDomain)
				.toList();
	...
	{
		return costoTratamientoJpaRepository.findAll().stream().map(CostoTratamientoDboMapper::toDomain).toList();
	}
}
