...
public class ClinicaMysqlDao implements SysSaludDao
{
	...
	{
		return sysSaludJpaRepository.findById(uuid).map(ClinicaDboMapper::toDomain)
	...
	public ListResponse<SysSalud> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<SysSalud> data = pageResult.getContent().stream().map(ClinicaDboMapper::toDomain).toList();
	...
	{
		return sysSaludJpaRepository.findAll().stream().map(ClinicaDboMapper::toDomain).toList();
	}
}
