...
public class PConsentimientoMysqlDao implements PConsentimientoDao
{
	...
	{
		return pConsentimientoJpaRepository.findById(uuid).map(PConsentimientoDboMapper::toDomain)
	...
	public ListResponse<PacienteConsentimiento> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<PacienteConsentimiento> data = pageResult.getContent().stream().map(PConsentimientoDboMapper::toDomain)
				.toList();
	...
	{
		return pConsentimientoJpaRepository.findAll().stream().map(PConsentimientoDboMapper::toDomain).toList();
	}
}
