...
public class RecetaMysqlDao implements RecetaDao
{
	...
	{
		return recetaJpaRepository.findById(uuid).map(RecetaDboMapper::toDomain)
	...
	public ListResponse<Receta> getAll(UUID hospitalId, int page, int rows)
	{
		...
		List<Receta> data = pageResult.getContent().stream().map(RecetaDboMapper::toDomain).toList();
	...
	{
		return recetaJpaRepository.findAll().stream().map(RecetaDboMapper::toDomain).toList();
	}
}
